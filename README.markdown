...
The actual source code is located in the `/src` folder, and their copies in the root and other directories are builds of the
scripts generated by gulp.

When making changes to the code, you must modify the files in `/src`.
When merging sources, you must run:
```shell
npm run clean
```
to clear all copies, then resolve any conflicts in the `/src` folder, and finally rebuild all sources via `gulp make`.
